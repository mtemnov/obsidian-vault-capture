...
const DESTINATIONS = {
  ...
  personal: {
    ...
    folderId: "REPLACE_WITH_PERSONAL_FOLDER_ID",
  },
};
...
function buildMarkdown(fields) {
  ...
  lines.push("tags:");
  fields.tags.forEach(function (t) {
    lines.push("  - " + t);
  });
  lines.push("---");
  lines.push("");
...
function successPage(destLabel, filename) {
  var style =
    ...
    ".icon{font-size:56px;margin-bottom:20px;color:#3ecf8e}" +
    "h1{font-size:28px;font-weight:700;color:#3ecf8e;margin-bottom:10px}" +
    "p{font-size:16px;color:#6b6b7b;margin:4px 0}" +
    ".file{font-family:monospace;font-size:13px;color:#9898a8;margin-top:12px;" +
    "background:#18181d;padding:8px 14px;border-radius:8px;display:inline-block;" +
    "max-width:90%;word-break:break-all}" +
    ".bar{position:fixed;bottom:0;left:0;height:4px;background:#3ecf8e;" +
